...
实现逻辑：通过Wxlogin接口实现微信用户注册与登录，首次登录和未绑定的账户（通过bind在前端判断）需要调用原来接口输入用户名和密码通过外键userid实现绑定，用户密码登录通过token进行身份验证，微信登录通过Wxtoken进行身份验证，绑定过的微信账户进行微信登录不需再输入用户名和密码
...
判断防止wxtoken通过Security组件解析失败

![image-20241028114814078](C:\Users\DELL\AppData\Roaming\Typora\typora-user-images\image-20241028114814078.png)
